  ...
  )
{
  ...
  RegEax = PcdGet32 (PcdPcIoApicEnable);
  DEBUG ((DEBUG_WARN, "%a: %d\n", __FUNCTION__, __LINE__));
  RegEax = PcdGet32 (PcdSupermicroTestUINT32);
  DEBUG ((DEBUG_WARN, "%a: %d\n", __FUNCTION__, __LINE__));
  TestPtr = (CHAR16*) PcdGetPtr(PcdMemInterposerMap);
  DEBUG ((DEBUG_WARN, "%a: %d\n", __FUNCTION__, __LINE__));
  TestPtr = (CHAR16*) PcdGetPtr(PcdSupermicroTestPtr);
  DEBUG ((DEBUG_WARN, "%a: %d\n", __FUNCTION__, __LINE__));
  TestPtr = (CHAR16*) PcdGetPtr(PcdSocketIioConfig);
  DEBUG ((DEBUG_WARN, "%a: %d\n", __FUNCTION__, __LINE__));
  CopyMem (&mSocketIioConfiguration, PcdGetPtr(PcdSocketIioConfig), sizeof(SOCKET_IIO_CONFIGURATION));
  DEBUG ((DEBUG_WARN, "%a: %d\n", __FUNCTION__, __LINE__));
  CopyMem (&mSocketMpLinkConfiguration, PcdGetPtr(PcdSocketMpLinkConfig), sizeof(SOCKET_MP_LINK_CONFIGURATION));
  DEBUG ((DEBUG_WARN, "%a: 2\n", __FUNCTION__));
  CopyMem (&mSocketPowermanagementConfiguration, PcdGetPtr(PcdSocketPowerManagementConfig), sizeof(SOCKET_POWERMANAGEMENT_CONFIGURATION));
  DEBUG ((DEBUG_WARN, "%a: 3\n", __FUNCTION__));
  CopyMem (&mSocketIioConfiguration, PcdGetPtr(PcdSocketIioConfig), sizeof(SOCKET_IIO_CONFIGURATION));
  DEBUG ((DEBUG_WARN, "%a: 4\n", __FUNCTION__));
  CopyMem (&mPchRcConfiguration, PcdGetPtr(PcdPchRcConfigurationData), sizeof(PCH_RC_CONFIGURATION));
  DEBUG ((DEBUG_WARN, "%a: 5\n", __FUNCTION__));
  ...
  )
{
  ...
  DEBUG ((DEBUG_WARN, "InstallAcpiBoard 1\n"));
  if (EFI_ERROR (Status)) {
    mIioUds = NULL;
    DEBUG ((DEBUG_WARN, "gEfiIioUdsProtocolGuid not found\n"));
  }
  // ASSERT_EFI_ERROR (Status);
  DEBUG ((DEBUG_WARN, "InstallAcpiBoard 2\n"));

  GuidHob    = GetFirstGuidHob (&gEfiMemoryMapGuid);
  DEBUG ((DEBUG_WARN, "InstallAcpiBoard 3\n"));
  ASSERT (GuidHob != NULL);
  if (GuidHob == NULL) {
    return EFI_NOT_FOUND;
  }
  DEBUG ((DEBUG_WARN, "InstallAcpiBoard 4\n"));
  mSystemMemoryMap = GET_GUID_HOB_DATA(GuidHob);

  DEBUG ((DEBUG_WARN, "InstallAcpiBoard 5\n"));
  PlatformHookInit ();
  DEBUG ((DEBUG_WARN, "InstallAcpiBoard 6\n"));
  //
  ...
  //
  DEBUG ((DEBUG_WARN, "InstallAcpiBoard 7\n"));
  ...
  Status    = EFI_SUCCESS;
  Instance  = 0;
  DEBUG ((DEBUG_WARN, "InstallAcpiBoard 8\n"));
